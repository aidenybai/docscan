            ...
            try:
                ...
                for paragraph in tree.getiterator(PARA):
                    texts = [node.text for node in
                             paragraph.getiterator(TEXT) if node.text]
                    if texts:
...
'''.join(paragraphs)
    ...
    def executeHeaderRegex(self, regularExpression):
        ...
        if self.fileFormat != 'docx':
            return '[WARN]   Cannot check header, ' + self.fileFormat \
                + ' has no XML properies'
        elif self.fileFormat.lower() == 'docx':
            listOfRegex = []
            count = 1
            while len(listOfRegex) <= 0 and count < 3:
                headerText = self.returnFileText('word/header'
                        + str(count) + '.xml')
    ...
    def executeFooterRegex(self, regularExpression):
        ...
        if self.fileFormat != 'docx':
            return '[WARN]   Cannot check footer, File Format "' \
                + self.fileFormat + '" has no XML properies'
        elif self.fileFormat.lower() == 'docx':
            listOfRegex = []
            count = 1
            while len(listOfRegex) <= 0 and count < 3:
                footerText = self.returnFileText('word/footer'
                        + str(count) + '.xml')
